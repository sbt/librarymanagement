...
class ManagedLoggerSpec extends FlatSpec with Matchers {
  ...
  it should "allow registering Show[Int]" in {
    ...
    val log = LogExchange.logger("foo")
    LogExchange.bindLoggerAppenders("foo", List(LogExchange.asyncStdout -> Level.Info))
    implicit val intShow: ShowLines[Int] = ShowLines({ (x: Int) =>
      Vector(s"String representation of $x")
    })
  ...
  it should "allow registering Show[Array[Int]]" in {
    ...
    val log = LogExchange.logger("foo")
    LogExchange.bindLoggerAppenders("foo", List(LogExchange.asyncStdout -> Level.Info))
    implicit val intArrayShow: ShowLines[Array[Int]] = ShowLines({ (x: Array[Int]) =>
      Vector(s"String representation of ${x.mkString}")
    })
  ...
  it should "allow registering Show[Vector[Vector[Int]]]" in {
    ...
    val log = LogExchange.logger("foo")
    LogExchange.bindLoggerAppenders("foo", List(LogExchange.asyncStdout -> Level.Info))
    implicit val intVectorShow: ShowLines[Vector[Vector[Int]]] = ShowLines({
      (xss: Vector[Vector[Int]]) =>
        Vector(s"String representation of $xss")
    })
