...
object ResolverTest extends UnitSpec {
  ...
  "Resolver url" should "propagate pattern descriptorOptional and skipConsistencyCheck." in {
    ...
    val patterns = ResolverUtil.url("test", new URL("http://foo.com/test"))(Patterns(pats, pats, isMavenCompatible = false, descriptorOptional = true, skipConsistencyCheck = true)).patterns
