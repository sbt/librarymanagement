...
trait InlineIvyConfigurationFormats { self: sbt.internal.librarymanagement.formats.LoggerFormat with sbt.internal.librarymanagement.formats.GlobalLockFormat with sbt.librarymanagement.ivy.formats.UpdateOptionsFormat with sbt.librarymanagement.ResolverFormats with sbt.librarymanagement.ModuleConfigurationFormats with sjsonnew.BasicJsonProtocol =>
implicit lazy val InlineIvyConfigurationFormat: JsonFormat[sbt.librarymanagement.ivy.InlineIvyConfiguration] = new JsonFormat[sbt.librarymanagement.ivy.InlineIvyConfiguration] {
  override def read[J](jsOpt: Option[J], unbuilder: Unbuilder[J]): sbt.librarymanagement.ivy.InlineIvyConfiguration = {
    jsOpt match {
      ...
      unbuilder.endObject()
      sbt.librarymanagement.ivy.InlineIvyConfiguration(log, lock, baseDirectory, ivyHome, updateOptions, resolvers, otherResolvers, moduleConfigurations, checksums, managedChecksums, resolutionCacheDir)
      case None =>
